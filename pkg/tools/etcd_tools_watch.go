...
// if the object should remain in the set.
...
// WatchList begins wathing the specified key's items. Items are decoded into
// API objects, and any iterms passing 'filter' are send down the returnd
// watch.Interface. resourceVersion may be used to specify what version to begin
// watching (e.g., for reconnecting wighout missing any updates.)
...
// API objects and send down the returnd watch.Interface.
...
// WatchAndTransform begins wathing the specified key. Events are decoded into API
// objects and send down the returnd watch.Interface. If the transform
// function is provided, the value decoded from etcd will be passed to the function
// prior to being returnd.
//
...
func (w *etcdWatcher) etcdWatch(client EtcdGetSet, key string, resourceVersion uint64) {
	...
	if resourceVersion == 0 {
		...
		_, err := client.Watch(key, resourceVersion, w.list, w.etcdIncoming, w.etcdStop) // XXX(ryutah): Maybe blocking here.
...
func convertRecursiveResponse(node *etcd.Node, response *etcd.Response, incoming chan<- *etcd.Response) {
	if node.Dir {
		for _, n := range node.Nodes {
			convertRecursiveResponse(n, response, incoming)
		}
	}
	copied := *response
	copied.Action = "get"
...
func (w *etcdWatcher) sendModify(res *etcd.Response) {
	...
	// Do nothing if neigher new nor old object passed the filter.
}
