...
func (p *ProxyHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	storage, ok := p.storage[resourceName]
	if !ok {
		httplog.LogOf(req, w).Addf("'%v' has not storage object", resourceName)
		notFound(w, req)
		return
	}
	...
	if err != nil {
		...
		writeJSON(status.Code, p.codec, status, w)
		return
	}
	...
	proxy.Transport = &proxyTransport{
		...
		proxyPathPrepend: path.Join(p.prefix, resourceName, id),
	}
...
// relative links work.
func (p *proxyTransport) updateURLs(n *html.Node, sourceURL *url.URL) {
	...
	for i, attr := range n.Attr {
		...
		if url.Host == "" {
			url.Scheme = p.proxyScheme
			url.Host = p.proxyHost
			url.Path = path.Join(p.proxyPathPrepend, path.Dir(sourceURL.Path), url.Path, "/")
			n.Attr[i].Val = url.String()
		} else if url.Host == sourceURL.Host {
			url.Scheme = p.proxyScheme
			url.Host = p.proxyHost
			url.Path = path.Join(p.proxyPathPrepend, url.Path)
			n.Attr[i].Val = url.String()
		}
	}
}
...
func (p *proxyTransport) scan(n *html.Node, f func(*html.Node)) {
	f(n)
...
func (p *proxyTransport) fixLinks(req *http.Request, resp *http.Response) (*http.Response, error) {
	...
	p.scan(doc, func(n *html.Node) { p.updateURLs(n, req.URL) })
