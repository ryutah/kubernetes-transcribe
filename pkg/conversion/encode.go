...
// will be made, therefore it's recommanded to pass a pointer to a struct.
...
//       within, and thir Version and Kind must both be empty.
//     * Note that the exception does not apply to a generig APIObject type
...
//    a native object, Encode will convert it to versioned object. For
...
// objects, whether they be in our storate layer (e.g., etcd), or in user's
// config files.
func (s *Scheme) EncodeToVersion(obj interface{}, destVersion string) (data []byte, err error) {
	obj = maybeCopy(obj)
	v, _ := enforcePtr(obj) // maybeCopy gurantees a pointer
