...
// MetaInsertionFactory is used to create an object to store and retrive
...
// factory is used to clear the version and tkind fields in memory, so it
// must match the layout of your actual api struct. (E.g., if you have your
...
// Scheme defines an entire encodeing and decoding scheme.
type Scheme struct {
	...
	// Indent will cause the JSON output from Encode to be indented, if it is true.
	Indent bool

	// InternalVersion is the default intenal version. It is recommanded that
	// you use "" for the internal version.
	InternalVersion string

	// MetaInsertionFactory is used to create an object to store and retrive
	// the version and kind informatin for all objects. The default uses the
	// keys "version" and "kind" respectively.
	MetaInsertionFactory MetaInsertionFactory
}

// NewScheme manufactores a new shceme.
...
// nameFunc returns the name of the type that we wish to use for encodeing. Defaults to
...
// the struct becomes the "kind" field when encodeing.
...
//		s.Convert(&in.SubFieldThatMoved, &out.NameLocation.NewName, 0)
//		return nil
//	}
// )
//
...
// possile. You can call this with types that haven't been registered(for example,
...
// have SrcVersion or DestVersion fields set currectly in Meta().
...
// MetaInsertionFactory provides a default implementation of MetaInsertionFactory
...
// This object will be used to encode this metadata along with your
// API objects, so the tags on the fields you use shouldn't conflict.
func (metaInsertion) Create(version string, kind string) interface{} {
	m := metaInsertion{}
	m.Version = version
	m.Kind = kind
	return &m
...
// Interpret should take the same type of object that Create creates.
// It should return the version and kind information from this objects.
func (metaInsertion) Interpret(in interface{}) (version string, kind string) {
	m := in.(*metaInsertion)
...
// maybeCopy copies obj if it is not a pointer, to get a settawble/addressable
...
// of the dereferenced pointer, ensuring that it it settable/addressable.
// Returns an erorr if this is not possible.
