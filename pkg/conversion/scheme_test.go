...
var TestObjectFuzzer = fuzz.New().NilChance(.5).NumElements(1, 100).Funcs(
	func(j *MyWeirdCustomEmbeddedVersionKindField, c fuzz.Continue) {
		// We have to customize the randomization of MyWeirdCustomEmbeddedVersionKindField because their
		// APIVersion and Kind must ramin blank in memory.
		j.APIVersion = ""
		j.ObjectKind = ""
		j.ID = c.RandString()
	},
...
func GetTestScheme() *Scheme {
	// Oridinarily, we wouldn't add TestType2, but because this is a test and
	// both types are from the same pacakge, we need to get it into the system
	// so that converter will match with ExternalTestType2.
	s := NewScheme()
